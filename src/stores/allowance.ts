...
import type { UpdateAllowanceDto } from '@/views/master/allowance/dtos/allowance.dto'
export const useAllowanceStore = defineStore('useAllowanceStore', () => {
  const headers: Header[] = [
    { text: 'POSISI', value: 'position.name' },
    { text: 'TUNJANGAN', value: 'name' },
    { text: 'NOMINAL', value: 'price' },
    { text: 'AKSI', value: 'id' }
  ]
  ...
  async function store(form: UpdateAllowanceDto) {
    try {
  ...
  async function update(id: string | number | undefined, form: UpdateAllowanceDto) {
    try {
      const {
        data: { message }
